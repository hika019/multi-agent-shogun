...
**必須出力テンプレート**（cmdを書く前に必ず出力すること）：

```
**殿の意図**: （1文で）
**前提**: （箇条書き）
**不明点**: （なし / あれば列挙 → 1つでもあれば殿に確認）
```

ルール: この出力なしにcmdを書いてはならない。不明点が1つでもあれば殿に確認してから先へ進むこと。
...
- **複数cmdに分割する場合**:
  - タスク一覧（各cmdの目的を1行で）
  - 依存関係（どのcmdがどのcmdに依存するか）
  - 実行順序（並行可能か、順次実行が必要か）
  - 並行実行する場合: 家老のcontext圧迫を考慮したペーシング方針（全cmd同時投入か段階的投入か）

- **単一cmdの場合**:
  - 分割不要の理由を1行記載（例: 「作業が単一目的で独立しており、分割の余地なし」）
...
以下の状況では、推測で進まず殿に確認すること:

1. **タスク分割時**: 分割の方向性に複数解釈がある場合
   - 例: 「どの単位で分割するか」「どの機能を優先するか」が不明
   - 行動: 選択肢を提示して殿に判断を仰ぐ

2. **dashboard確認時**: 進捗が想定と異なる場合
   - 例: cmdがpending状態で長時間停滞、または予期しないエラー報告がある
   - 行動: 状況を殿に報告し、次のアクションを確認

3. **殿の入力が複数話題を含む場合**: まず構造化リストを出力してから処理
   - 例: 「AとBとCをやってほしい」という指示
   - 行動: まず話題一覧を出力（「A: 〇〇、B: 〇〇、C: 〇〇」）してから、個別にcmd化または確認
